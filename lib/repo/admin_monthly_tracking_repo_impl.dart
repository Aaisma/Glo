...
import 'admin_monthly_tracking_repo.dart';
...
class AdminMonthlyTrackingRepoImpl implements AdminMonthlyTrackingRepo {
  ...
  Future<Map<String, dynamic>> getPeriodAnalytics(DateTime startDate, DateTime endDate) async {
    ...
    for (var userLogs in logsByUser.values) {
      ...
      for (int i = 0; i < userLogs.length; i++) {
        ...
        if (log.isPeriodDay) {
          currentPeriodLength++;
          // Gap threshold matches CycleAnalyticsEngine's period grouping (<=3 days is
          // still the same period, to allow for spotting/pauses), so cycle counts here
          // stay consistent with the ones used for ovulation/symptom phase analytics.
          if (i == 0 || userLogs[i - 1].date.difference(log.date).inDays.abs() > 3) {
    ...
    // No fallback to industry-average numbers (28/5) when there's no real data -
    // an empty dataset should show as 0, not a made-up value that looks real.
    double avgCycleLength = cycleLengths.isEmpty ? 0 : cycleLengths.reduce((a, b) => a + b) / cycleLengths.length;
    double avgPeriodLength = periodLengths.isEmpty ? 0 : periodLengths.reduce((a, b) => a + b) / periodLengths.length;
  ...
  // Returns a map keyed by "yyyy-MM-dd" -> {hasPeriod, hasOvulation, hasSymptoms, totalLogsUsers}
  // for every day in [monthStart, monthEndExclusive) that has at least one real log.
  // Days with no entry in the returned map have no logs at all - the UI should treat
  // "missing key" as "no marker", never assume a default.
  @override
  Future<Map<String, Map<String, dynamic>>> getCalendarMonthSummary(
      DateTime monthStart, DateTime monthEndExclusive) async {
    final periodSnap = await _firestore
        .collection('period')
        .where('date', isGreaterThanOrEqualTo: monthStart.toIso8601String())
        .where('date', isLessThan: monthEndExclusive.toIso8601String())
        .get();
    ...
    // userIds tracked separately per day so totalLogsUsers reflects unique users, not doc count.
    final Map<String, Map<String, dynamic>> raw = {};
    final Map<String, Set<String>> userIdsByDay = {};

    Map<String, dynamic> entryFor(String key) {
      return raw.putIfAbsent(key, () => {
        "hasPeriod": false,
        "hasOvulation": false,
        "hasSymptoms": false,
      });
    ...
    for (var doc in periodSnap.docs) {
      ...
      entry["hasPeriod"] = true;
      final symptoms = Map<String, bool>.from(data['symptoms'] ?? {});
      if (symptoms.values.any((v) => v)) entry["hasSymptoms"] = true;
      final userId = data['userId']?.toString();
      if (userId != null) userIdsByDay.putIfAbsent(key, () => {}).add(userId);
    }
    ...
    return raw.map((key, value) {
      return MapEntry(key, {
        ...value,
        "totalLogsUsers": userIdsByDay[key]?.length ?? 0,
      });
    });
  }
}